 ...
 * TeacherDashboard
 * 
 * Refactored version using custom hooks and common components.
 */

import React, { useState, useEffect } from 'react';
import { Quiz, StudentResult, ImageLibraryItem, QuestionType } from '../types';
import { AIProvider, generateQuiz, QuizGenerationOptions } from '../geminiService';
import { LogOut, FileText, List, Settings, RefreshCw, Download, Bot, Loader2, Save, Trash2, Edit, Search, Sparkles, PlusCircle, Key, X } from 'lucide-react';
...
import { Tabs, TabItem, Card, Button } from '../src/components/common';
...
import { useResults, useQuizManager } from '../src/hooks';
...
const TeacherDashboardV2: React.FC<Props> = ({
    ...
    // Quiz Creator state (keeping inline for now, can migrate later)
    ...
    const [customPrompt, setCustomPrompt] = useState(''); // Custom prompt từ giáo viên
    const [quizMode, setQuizMode] = useState<'exam' | 'practice'>('practice'); // Chế độ tạo đề
    ...
    const handleUpdateAccessCode = async () => {
        ...
        const quiz = quizzes.find(q => q.id === editingAccessCode.quizId);
        if (!quiz) return;
        ...
        const updatedQuiz = {
            ...
            requireCode: hasCode, // Tự động set TRUE khi có mã, FALSE khi xóa mã
        };
    ...
    const renderResultsTab = () => (
        <div className="space-y-6">
            ...
            {/* Results Table - Using new component */}
            <Card padding="none">
                <ResultsTable
                                ...
                                <Button
                                    onClick={() => {
                                        setEditingAccessCode({
                                            quizId: quiz.id,
                                            currentCode: quiz.accessCode || '',
                                        });
                                        setNewAccessCode(quiz.accessCode || '');
                                    }}
                                    variant="ghost"
                                    size="sm"
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const options: QuizGenerationOptions = {
                ...
                // Exam mode: custom prompt is required and prioritized
                // Practice mode: custom prompt is optional
            ...
            const quiz: Quiz = {
                ...
                createdAt: editingQuizId ? generatedQuiz?.createdAt || new Date().toISOString() : new Date().toISOString(),
    ...
    const handleSaveQuiz = async () => {
        ...
        try {
            ...
            setContent('');
            setCustomPrompt('');
            setRequireCode(false); // Reset access code
            setAccessCode('');
    ...
    // Render Create Tab - Full Implementation
    const renderCreateTab = () => {
        ...
        return (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Left Column - Form */}
                <div className="space-y-4">
                    <Card title={editingQuizId ? '📝 Chỉnh sửa đề' : '✨ Tạo đề kiểm tra mới'}>
                        <div className="space-y-4">
                            {/* Basic Info */}
                            ...
                            {/* Custom Prompt - Yêu cầu đặc biệt từ giáo viên */}
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">
                    ...
                    <Card title="📋 Xem trước đề thi">
                        {generatedQuiz ? (
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h3 className="font-bold text-lg">{generatedQuiz.title}</h3>
                                        <p className="text-sm text-gray-500">
                                            Lớp {generatedQuiz.classLevel} • {generatedQuiz.questions.length} câu • {generatedQuiz.timeLimit} phút
                                        </p>
                                    </div>
                                    <Button
