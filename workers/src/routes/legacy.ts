...
export async function handleLegacyAction(db: D1Database, action: string, body: any): Promise<Response> {
    switch (action) {
        ...
        case 'create_quiz':
            return await handleCreateQuiz(db, body);

        case 'update_quiz':
            return await handleUpdateQuiz(db, body);
        ...
        case 'add_student':
            return await handleAddStudent(db, body);
        ...
        case 'reset_student_password':
            return await handleResetPassword(db, body);

        case 'student_login':
            return await handleStudentLogin(db, body);
        ...
        case 'get_assignments': {
            ...
            const rows = await db.prepare('SELECT * FROM assignments WHERE class_id = ?').bind(body.classId).all<import('../types').Assignment>();
            return jsonResponse({ status: 'success', data: mapAssignments(rows.results) });
        }

        case 'get_teacher_assignments':
            return await handleGetTeacherAssignments(db, body);
        ...
        case 'get_student_assignments':
            return await handleGetStudentAssignments(db, body);
        ...
        case 'update_game_state':
            return await handleUpdateGameState(db, body);

        case 'buy_shop_item':
            return await handleBuyShopItem(db, body);
...
async function handleCreateQuiz(db: D1Database, body: any) {
    await db.prepare(
...
async function handleUpdateQuiz(db: D1Database, body: any) {
...
async function handleAddStudent(db: D1Database, body: any) {
...
async function handleResetPassword(db: D1Database, body: any) {
...
async function handleStudentLogin(db: D1Database, body: any) {
    const enc = new TextEncoder();
...
async function handleGetTeacherAssignments(db: D1Database, body: any) {
    await db.prepare(`UPDATE assignments SET status = 'CLOSED' WHERE status = 'OPEN' AND deadline < ?`).bind(new Date().toISOString()).run();
    const teacherClasses = await db.prepare('SELECT id FROM classes WHERE teacher_username = ?').bind(body.teacherUsername).all();
...
async function handleGetStudentAssignments(db: D1Database, body: any) {
    ...
    if (asns.results.length === 0) return jsonResponse({ status: 'success', data: [] });
...
async function handleStartAssignmentAttempt(db: D1Database, body: any) {
...
async function handleGetPetData(db: D1Database, body: any) {
...
async function handleUpdateGameState(db: D1Database, body: any) {
    if (!body.username) return jsonResponse({ status: 'error', message: 'Missing username' });
...
async function handleBuyShopItem(db: D1Database, body: any) {
