...
export function buildCertificateSvg(bgHref: string, fieldsConfig: FieldConfig[], data: CertificateTextData, width: number, height: number): string {
  const textNodes = fieldsConfig.map((field) => {
    ...
    return `<text x="${field.x}" y="${field.y}" font-family="Roboto" font-size="${field.fontSize ?? 32}" font-weight="${field.fontWeight === 'bold' ? '700' : '400'}" fill="${field.color ?? '#000000'}" text-anchor="${anchor}" dominant-baseline="middle"${maxWidthAttr}>${escapeXml(String(rawValue))}</text>`;
  }).join('');
