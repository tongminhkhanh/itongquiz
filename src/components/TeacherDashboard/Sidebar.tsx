...
const Sidebar: React.FC<SidebarProps> = ({
                        ...
                        {items.map((item) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-5 py-3 transition-all duration-200 border-l-4 rounded-r-xl ${isActive
    ...
    return (
        <>
            ...
            <aside
                className={`fixed top-0 left-0 z-50 h-[calc(100vh-64px)] lg:h-full pb-20 lg:pb-0 w-64 bg-slate-50 border-r border-slate-200 flex flex-col transition-transform duration-300 ease-in-out overflow-y-auto ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
                    }`}
            >
